...
## PURPOSE : Compute milenage algorithm following 3GPP 35.206
## SPECIAL NOTES: $1: Optkey
...
void usage(void)
{
  printf("usage (two modes, normal and failure):\n");
  printf("    OP K RAND [SQN xor AK] [AMF]\n");
  printf("        optional parameters as seen in auth request\n");
  printf("    -f OP K RAND [SQN_MS xor AK] [AMF*: you should use '0000']\n");
  printf("        optional parameters as seen in auth failure (rand is in auth request)\n");
  printf("        (I should not include AMF* as a parameter, it is 0000, but\n");
  printf("        I could not find the spec where it is defined, so...)\n");
  exit(1);
}
...
int main(int n, char **v)
{
  ...
  for (i = 1; i < n; i++) {
    ...
    if (!strcmp(v[i], "-f")) { mode_failure = 1; continue; }
    ...
    if (_amf == NULL)  { _amf  = v[i]; continue; }
    usage();
  }

  if (_op == NULL || _k == NULL || _rand == NULL) usage();

  op = add(_op, op, &oplen);
  k = add(_k, k, &klen);
  rand = add(_rand, rand, &randlen);

  if (oplen != 16 || klen != 16 || randlen != 16) {
    printf("bad op k or rand, length must be 16 bytes\n");
    exit(1);
  }
  ...
  if (_sqn_xor_ak != NULL) {
    ...
    if (sqn_xor_aklen != 6) {
      printf("bad SQN xor AK, must be 6 bytes\n");
      exit(1);
    }
  ...
  /* in case of auth failure, UE sends back sqn_ms xor "ak"
   * where "ak" is computed by f5star
   * see 33.105 5.1.1.3 "Generation of re-synchronisation token in the USIM"
   * let's print sqn_ms
   */
  if (mode_failure) {
    f5star(k, rand, ak);
    printf("ak from f5*(k, rand): ");
    for (i = 0; i < 6; i++) printf("%2.2x", ak[i]);
    printf("\n");
  }

  if (_sqn_xor_ak != NULL && mode_failure) {
    printf("sqn_ms (using ak=f5*(k, rand)): ");
    for (i = 0; i < 6; i++) printf("%2.2x", sqn_xor_ak[i] ^ ak[i]);
    printf("\n");
  }

  /* for AMF* you should use 0000
   * I don't know in what spec it is defined
   */
  if (_amf != NULL) {
    unsigned char sqn_ms[6];
    unsigned char mac_s[8];
    unsigned char mac_a[8];
    amf = add(_amf, amf, &amflen);
    if (amflen != 2) {
      printf("bad AMF, must be 2 bytes\n");
      exit(1);
    }

    if (mode_failure) {
      if (amf[0] != 0 || amf[1] != 0) { printf("WARNING: AMF* is not 0000\n"); }
      for (i = 0; i < 6; i++) sqn_ms[i] = sqn_xor_ak[i] ^ ak[i];
      f1star(k, rand, sqn_ms, amf, mac_s);
      printf("mac-s: ");
      for (i = 0; i < 8; i++) printf("%2.2x", mac_s[i]);
      printf("\n");
    }
    ...
    if (!mode_failure) {
      ...
      printf("mac-a: ");
      for (i = 0; i < 8; i++) printf("%2.2x", mac_a[i]);
      printf("\n");
      printf("sqn: ");
      for (i = 0; i < 6; i++) printf("%2.2x", sqn[i]);
      printf("\n");
